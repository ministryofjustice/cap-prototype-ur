...
var howChangeDuringSchoolHolidays_exports = {};
__export(howChangeDuringSchoolHolidays_exports, {
  default: () => howChangeDuringSchoolHolidays_default
});
module.exports = __toCommonJS(howChangeDuringSchoolHolidays_exports);
...
const getFieldName = (childIndex) => `${import_formFields.default.HOW_CHANGE_DURING_SCHOOL_HOLIDAYS}-${childIndex}`;
...
const howChangeDuringSchoolHolidaysRoutes = (router) => {
  router.get(import_paths.default.HANDOVER_HOLIDAYS_HOW_CHANGE_DURING_SCHOOL_HOLIDAYS, (0, import_checkFormProgressFromConfig.default)(import_formSteps.default.HANDOVER_HOLIDAYS_HOW_CHANGE_DURING_SCHOOL_HOLIDAYS), (request, response) => {
    ...
    const sessionHAH = isD2 ? (0, import_perChildSession.getSessionValue)(request.session, "handoverAndHolidays") : handoverAndHolidays;
  ...
  router.post(
    import_paths.default.HANDOVER_HOLIDAYS_HOW_CHANGE_DURING_SCHOOL_HOLIDAYS,
    (request, response, next) => {
      const { numberOfChildren } = request.session;
      const validations = [];
      validations.push(
        (0, import_express_validator.body)(getFieldName(0)).trim().notEmpty().withMessage((_value, { req }) => req.__("handoverAndHolidays.howChangeDuringSchoolHolidays.error"))
      );
      for (let i = 1; i <= numberOfChildren; i++) {
        const fieldName = getFieldName(i);
        if (request.body[fieldName] !== void 0 && request.body[fieldName] !== "") {
          validations.push(
            (0, import_express_validator.body)(fieldName).trim().notEmpty().withMessage((_value, { req }) => req.__("handoverAndHolidays.howChangeDuringSchoolHolidays.error"))
          );
        }
      }
    ...
    (request, response) => {
      ...
      if ((0, import_perChildSession.isDesign2)(request.session)) {
        ...
        (0, import_perChildSession.setSessionSection)(request.session, "handoverAndHolidays", { ...currentHAH, howChangeDuringSchoolHolidays: newHowChange });
      } else {
      ...
      (0, import_addCompletedStep.default)(request, import_formSteps.default.HANDOVER_HOLIDAYS_HOW_CHANGE_DURING_SCHOOL_HOLIDAYS);
      if ((0, import_perChildSession.isDesign2)(request.session)) {
        if (request.body["apply-to-all"] === "yes") {
          ...
          for (let i = 0; i < numberOfChildren; i++) {
            if (i !== savedIndex) {
              ...
              (0, import_perChildSession.setSessionSection)(request.session, "handoverAndHolidays", { ...childHAH, howChangeDuringSchoolHolidays: newHowChange });
            }
          }
          request.session.currentChildIndex = 0;
  ...
  router.post(import_paths.default.HANDOVER_HOLIDAYS_HOW_CHANGE_DURING_SCHOOL_HOLIDAYS_NOT_REQUIRED, (request, response) => {
    request.session.handoverAndHolidays = {
      ...request.session.handoverAndHolidays,
...
var howChangeDuringSchoolHolidays_default = howChangeDuringSchoolHolidaysRoutes;
//# sourceMappingURL=howChangeDuringSchoolHolidays.js.map
